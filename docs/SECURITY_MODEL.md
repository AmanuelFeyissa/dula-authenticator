...
|---|---|---|---|---|---|---|
| OS-backed secret storage | Keystore | Keychain | DPAPI | Keychain | libsecret — **requires a running keyring daemon** ([ADR-0004](adr/0004-secure-storage-linux-keyring.md)). Every call is gated behind a bounded D-Bus check for `org.freedesktop.secrets`; with no daemon the app shows a blocking error within seconds, and an unreachable store is never mistaken for an empty one ([ADR-0018](adr/0018-linux-secret-service-gate.md)). The gate needs `gdbus` or `dbus-send` on `PATH`; with neither it opens and the pre-ADR-0018 hang is possible | **None** |
