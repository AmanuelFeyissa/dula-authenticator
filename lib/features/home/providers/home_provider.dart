...
class AccountListNotifier extends StateNotifier<AsyncValue<List<OtpAccount>>> {
  ...
  AccountListNotifier(this._repository, this._ref)
  ...
  Future<bool> addAccount(OtpAccount account) async {
    final authState = _ref.read(authStateProvider);
    final success = await _repository.addAccount(
      account,
      masterKey: authState.masterKey,
    );
    if (success) {
  ...
  Future<void> advanceCounter(String id) async {
    ...
    final updated = accounts[index].copyWith(
      counter: accounts[index].counter + 1,
    );
    final saved = await _repository.updateAccount(
      updated,
      masterKey: authState.masterKey,
    );
  ...
  Future<bool> updateAccount(OtpAccount account) async {
    final authState = _ref.read(authStateProvider);
    final success = await _repository.updateAccount(
      account,
      masterKey: authState.masterKey,
    );
...
final accountListProvider =
    StateNotifierProvider<AccountListNotifier, AsyncValue<List<OtpAccount>>>((
      ref,
    ) {
      final notifier = AccountListNotifier(
